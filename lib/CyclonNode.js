...
function CyclonNode(neighbours, numNeighbours, shuffleSize, comms, bootstrapper, tickIntervalMs, metaDataProviders, asyncExecService, logger, storage) {
    ...
    this.createNewPointer = function () {
        ...
        if (metaDataProviders) {
            for (var metaDataKey in metaDataProviders) {
                metaData[metaDataKey] = metaDataProviders[metaDataKey]();
            }
        }

        return comms.createNewPointer(metaData);
    };
